...
def train_and_evaluate_knn(x_train, x_test, y_train, y_test):
    ...
    save_path = os.path.join(project_root, 'results', 'Knn', 'confusion_matrix.png')
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, format='png')
    metrics_results = evaluate_model(y_test, y_pred, save_path=save_path)

    # Report di classificazione
    print("\n","REPORT DI CLASSIFICAZIONE KNN", "\n")
    ...
    shap_path = os.path.join(project_root, 'results', 'Knn', 'shap.png')
