...
def train_and_evaluate_random_forest(x_train, x_test, y_train, y_test,seed):
    ...
    save_path = os.path.join(project_root, 'results', 'Random Forest', 'confusion_matrix.png')
    ...
    for method, selector in feature_selection_methods.items():
        ...
        RandomForest = RandomForestClassifier(random_state=seed)  # Sostituisci con il classificatore desiderato
        ...
        # Valuta il modello
        y_pred = RandomForest.predict(X_test_selected)
    ...
    shap_path = os.path.join(project_root, 'results', 'Random Forest', 'shap.png')
    ...
    model_selected = RandomForestClassifier(random_state=seed)
